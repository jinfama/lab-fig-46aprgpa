...
const FACET_MARGIN = { top: 18, right: 10, bottom: 24, left: 50 };
...
function _drawMiniLegend(g, series, w, maxRows = 4) {
    ...
    const legend = g.append('g').attr('transform', `translate(${w - 4}, 4)`);
    rows.forEach((s, i) => {
        const row = legend.append('g').attr('transform', `translate(0, ${i * 12})`);
        row.append('line')
            .attr('x1', -58).attr('x2', -47)
            .attr('y1', 0).attr('y2', 0)
            .attr('stroke', s.color)
            .attr('stroke-width', 2);
        row.append('text')
...
function _buildBilateralTrendSeries() {
    ...
    const yearRange = State.get('yearRange') || [years[0], years[years.length - 1]];
...
function _renderBilateralOverlay() {
    ...
    const xDomain = _xDomainForRange(State.get('yearRange'), allData);
...
function _renderOverlay() {
    ...
    overlay
        .on('mousemove', (event) => {
            ...
            const entries = [];
            allSeries.forEach((s, i) => {
                const d = s.data.find(d => d.year === year);
                if (d && d.value != null) {
                    hoverDots[i]
                        .attr('cx', x(year))
                        .attr('cy', y(d.value))
...
function _renderFacetByCountry() {
    ...
    const allData = panels.flatMap(panel => panel.series.flatMap(s => s.data));
    if (allData.length === 0) return;
...
function _renderFacetByProduct() {
    ...
    panels.forEach((panel, idx) => {
        ...
        if (panel.series.length > 1 && panel.series.length <= 4) {
            const legend = g.append('g').attr('transform', `translate(${w - 4}, 4)`);
            panel.series.forEach((series, i) => {
                const rowG = legend.append('g').attr('transform', `translate(0, ${i * 12})`);
                rowG.append('line')
                    .attr('x1', -52).attr('x2', -43).attr('y1', 0).attr('y2', 0)
                    .attr('stroke', series.color).attr('stroke-width', 2);
                rowG.append('text')
                    .attr('x', -38).attr('y', 3)
                    .attr('font-size', '8px')
                    .attr('fill', '#7A6A5A')
                    .text(_truncateFacetLabel(series.name, 11));
            });
        }
...
function _renderStacked() {
    ...
    overlay
        .on('mousemove', (event) => {
            ...
            const row = dataByYear.find(r => r.year === year);
            const entries = [];
            if (row) {
                // Find the matching stacked layer index for each series
                stacked.forEach((layer, i) => {
